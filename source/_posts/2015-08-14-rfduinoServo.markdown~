...
<br/>
Ok, after a bit of digging around, I discovered that although the RFduino BLE works well and although the RFduino Servo Shield also works well (in theory), they do not work well together.  The way the servo's are driven is by way of a Pulse Width Modulation.  In the above example, the RFduino is actually generating the PWM signal itself, but whenever it needs to service the Bluetooth, it switches tasks and leaves the PWM unattended.  This causes a large jitter.  The best PWM signals are generated by using a couple of counters, one for Pulse Width and the other for Pulse Duration.  If this were implemented with counters, then the RFduino would not be relied upon as a 'real time' device.  One look at the RFduino Servo Shield Schematic and you can see that indeed there is only a Voltage regulators and a quad amplifier.
...
The solution is to use a true servo shield which provides the proper PWM handling.  In that way, the controller would only need to write two registers (width, period) per servo.  Adafruit provides such a board which I have but its out of scope of this project.

*Note:  Another possible solution is to use the RFduino and servo shield without the BLE, but in my opinion, it is an inferior design because you are using a micro controller to perform very simple but real time hardware task.

###Conclusion
This post shows you how to connect an Iphone to and RFduino and servo shield.  I love how this project is so self contained.  You can easily imagine how the parts can easily fit into a robotic system which is not tethered to any stationary devices for power or control.  Although this project proved to be a tad flawed, if an improved servo shield was used, the project would be quite successful.  Maybe I will do this and update this writeup in the future, but for now...
